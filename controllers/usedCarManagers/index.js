...
const login = async (req, res, next) => {
  ...
  try {
    user = await usersModel.doLogin(email, password, config.get('usedCarMgrRole'));
...
const forgotPassword = async (req, res, next) => {
  ...
  let email = req.body.email;
  try {
    user = await usersModel.generateForgotToken(email, config.get('usedCarMgrRole'));
...
const sendMessage = async (req, res, next) => {
  smsServices.sendMessage(req.body.phone, req.body.message, function (sms, err) {
    if (sms.errorCode) return next(sms.errorCode);
    else if(sms!=undefined) {
      return res.sendResponse({}, 'Your message ' +sms.status);
    }
  });
};
