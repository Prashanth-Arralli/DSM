...
const login = async(req, res, next) => {
    ...
    try {
        users = await usersModel.doLogin(email, password, config.get('mrktngMgrRole'));
        return res.sendResponse(users, 'Marketing manager has successfully logged in.');
    } catch (ex) {
        return next(ex);
    }
};
...
const forgotPassword = async(req, res, next) => {
    ...
    let email = req.body.email;
    try {
        user = await usersModel.generateForgotToken(email, config.get('mrktngMgrRole'));
